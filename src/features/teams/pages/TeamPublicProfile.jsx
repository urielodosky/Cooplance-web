...
const TeamPublicProfile = () => {
    ...
    const isPendingMember = team.members.some(m => m.userId === user?.id && m.status === 'pending');
                        ...
                        {team.members.filter(m => m.status === 'left').length === 0 ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                                {team.members.filter(m => m.status === 'left').map(m => (
                                    <div key={m.userId} style={{ background: 'rgba(255,255,255,0.03)', padding: '0.8rem', borderRadius: '12px', fontSize: '0.9rem' }}>
                    ...
                    <div className="glass" style={{ padding: '1.5rem', borderRadius: '20px' }}>
                        ...
                        <div style={{ display: 'grid', gap: '1rem' }}>
                            {team.members.filter(m => m.status === 'active').map(member => (
                                <div key={member.userId} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.8rem', background: 'rgba(255,255,255,0.03)', borderRadius: '12px' }}>
